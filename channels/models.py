...
class Channel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.api_key:
            self.api_key = hashlib.sha256((str(self.owner.username) + str(random.random())).encode('utf-8')).hexdigest()[:7] + '-' + hashlib.sha1(str(self.pub_date).encode('utf-8')).hexdigest()[:7]
            self.save()